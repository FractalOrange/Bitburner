...
export async function main(ns) {
	...
	let contractTime = 5 * 60 * 1000
	let operationTime = 5 * 60 * 1000
	let workTime = 5 * 60 * 1000
	...
	while (true == true) {
		...
		for (let city of cities){
			if (ns.bladeburner.getCityChaos(city) > chaosThresh){
				while (ns.bladeburner.getCityChaos(city) > 0.9 * chaosThresh){
					await reduceChaos(city)
				}
			}
		}
		// Make sure our population estimates are accurate
		for (let city of cities) {
			while (await improveEstimate(city) > 0){}
		}
		...
		// Go to city and repeat the best contact for 5 minutes, unless all contracts in all cities are invalid.
		if (maxProfit !=0){
			if (ns.getPlayer().workType == "Working for Company"
			&& simulacrumBool == false){
				ns.stopAction()
			}
			let contractRepeats = Math.floor(contractTime / ns.bladeburner.getActionTime('contract', profitContract))
		...
		// Regardless of anything else, if we can easily do assassination, we should do that since it scales the best.
		...
		// Go to city and repeat the action, as long as the predicted rank gain is higher than the rankThresh
		// that we calculate from the rank gain for doing the optimal contract.
		if (maxRank > rankThresh){
			...
			let operationRepeats = Math.floor(operationTime / ns.bladeburner.getActionTime('operation', rankOperation))
		...
		// Check for black ops and do any that have over 50% success rate.
		...
		// If we have a job from a megacorp, work that for 5 minutes
		if (ns.getPlayer().companyName != '' && simulacrumBool == false){
			await ns.sleep(workTime)
		}
		// Wait a relatively long time to let megacorps get a job as needed.
		await ns.sleep(100)
	...
	async function improveEstimate(city){
		ns.bladeburner.switchCity(city)
		if (ns.getPlayer().workType == "Working for Company"
		&& simulacrumBool == false){
			ns.stopAction()
	...
	async function reduceChaos(city){
		ns.bladeburner.switchCity(city)
		if (ns.getPlayer().workType == "Working for Company"
		&& simulacrumBool == false){
			ns.stopAction()
